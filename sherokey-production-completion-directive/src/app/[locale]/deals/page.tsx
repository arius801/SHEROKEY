...
export default async function DealsPage({
  ...
  const sort = (sp.sort as ProductFilters["sort"]) || "discount";

  const filters: ProductFilters = { locale, onSale: true, sort, limit: PAGE_SIZE, offset: (page - 1) * PAGE_SIZE };

  const [{ items, total }, wishlistIds] = await Promise.all([listProducts(filters), getWishlistProductIds(ctx.user?.id ?? null)]);

  return (
    <ProductListing
      title={ctx.dict.home.specialOffers}
      subtitle={ctx.dict.home.viewDeals}
      items={items}
      total={total}
      page={page}
      pageSize={PAGE_SIZE}
      basePath={`/${locale}/deals`}
      searchParams={sp}
      sort={sort}
      ctx={ctx}
