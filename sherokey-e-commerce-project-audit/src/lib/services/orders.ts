...
/** Marks a pending order as failed/cancelled when a payment attempt does not succeed. Idempotent. */
export async function markOrderPaymentFailed(orderId: number, reason: string) {
  const rows = await db.select().from(orders).where(eq(orders.id, orderId)).limit(1);
  const order = rows[0];
  ...
  await logAudit({ action: "order.payment_failed", entityType: "order", entityId: orderId, metadata: { reason } });
}

/**
 * Server-authoritative refund: looks up the captured payment for an order, calls the
 * matching gateway's refund API (Stripe/PayPal), records a `refunds` row and flips the
 * order + payment status. Manual/bank-transfer/sandbox payments are refunded as an
 * internal bookkeeping action only (no external gateway call is possible for them).
 */
export async function refundOrderAdmin(params: { orderId: number; amountMinor?: number; reason?: string; adminUserId: number }) {
  const orderRows = await db.select().from(orders).where(eq(orders.id, params.orderId)).limit(1);
  const order = orderRows[0];
  if (!order) throw new OrderError("ORDER_NOT_FOUND", "Order not found");
  if (order.paymentStatus !== "paid") throw new OrderError("NOT_PAID", "Only paid orders can be refunded");
  ...
  const amountMinor = params.amountMinor ?? order.totalMinor;

  const { refundStripePayment } = await import("@/lib/services/payments/stripe");
  const { refundPaypalCapture } = await import("@/lib/services/payments/paypal");

  if (payment?.provider === "stripe") {
    const intentId = (payment.metadata as { paymentIntentId?: string } | null)?.paymentIntentId ?? payment.transactionId;
    await refundStripePayment(intentId, amountMinor);
  } else if (payment?.provider === "paypal") {
    const captureId = (payment.metadata as { captureId?: string } | null)?.captureId ?? payment.transactionId;
    await refundPaypalCapture(captureId, amountMinor, order.currency);
  }
  // bank_transfer / card_sandbox: recorded as a manual/internal refund only.

  await db.insert(refunds).values({
    orderId: order.id,
    paymentId: payment?.id,
    amountMinor,
    reason: params.reason ?? "Refunded by administrator",
    status: "processed",
  ...
  if (order.userId) {
    await notifyUser(order.userId, "order_refunded", "Order refunded", `Your order #${order.orderNumber} has been refunded.`, `/account/orders/${order.id}`);
  }
  await logAudit({ userId: params.adminUserId, action: "order.refunded", entityType: "order", entityId: order.id, metadata: { amountMinor, reason: params.reason } });

  return { orderId: order.id, amountMinor };
}
